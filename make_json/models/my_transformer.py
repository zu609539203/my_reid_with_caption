...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask):
        ...
        tgt = self.embeddings(tgt).permute(1, 0, 2)#问题就出在这
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False):
        super().__init__()
        # self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout) 和上面自定义类参数顺序一致
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
                                   ...
                                   mask=memory_key_padding_mask)[0]#mask改成自定义的名字
